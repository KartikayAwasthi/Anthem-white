...
const RoomPage = () => {
  ...
  // Callback handlers for 3D interactions
  const handleWallClickFrom3D = (wallType, color) => {
    setShowColorTransition(true);
    setTimeout(() => {
      switch(wallType) {
        case 'back':
  ...
  const handleFanClickFrom3D = (newModel) => {
    setFanModel(newModel);
  };

  const handleLightClickFrom3D = (newState) => {
    setIsBulbOn(newState);
  };
    ...
    <div className="min-h-screen bg-[#1c1c1c] text-white pt-16 sm:pt-20">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col lg:flex-row gap-6 mb-8">
          ...
          <div className="lg:flex-1 bg-[#2f2f2f] rounded-lg overflow-hidden order-1 lg:order-2 h-[50vh] sm:h-[60vh] lg:h-[70vh] relative cursor-grab active:cursor-grabbing">
            <Room3D 
              wallColor={wallColor}
              ...
              isBulbOn={isBulbOn}
              onWallClick={handleWallClickFrom3D}
              onFanClick={handleFanClickFrom3D}
              onLightClick={handleLightClickFrom3D}
            />
            
            {/* 3D Room Enhancement Overlay */}
            <div className="absolute top-4 left-4 bg-black/70 text-white px-3 py-2 rounded-lg text-xs backdrop-blur-sm border border-white/20">
              <div className="text-[#e49385] font-semibold mb-1">🎮 Interactive Mode</div>
              <div className="space-y-1">
                <div>Click elements to interact</div>
                <div>Hover for tooltips</div>
              </div>
            </div>
            
            {/* Performance indicator */}
            <div className="absolute top-4 right-4 bg-green-500/20 text-green-400 px-2 py-1 rounded text-xs font-mono border border-green-500/30">
              3D Loaded
            </div>
          </div>
            ...
            <div className="space-y-4 sm:space-y-6">
              ...
              <div className="mt-6 sm:mt-8 pt-4 sm:pt-6 border-t border-gray-600">
                ...
                <ul className="text-gray-300 text-xs sm:text-sm space-y-1">
                  <li>• <strong>3D Controls:</strong> Left click + drag to rotate</li>
                  <li>• <strong>Zoom:</strong> Mouse wheel to zoom in/out</li>
                  <li>• <strong>Pan:</strong> Right click + drag to pan</li>
                  <li>• <strong>Interactive:</strong> Click walls, fan, or light</li>
                  <li>• <strong>Panel Controls:</strong> Use controls for fine-tuning</li>
                  <li>• <strong>Hover:</strong> Hover over elements for tooltips</li>
                </ul>
                
                <div className="mt-4 p-3 bg-[#1c1c1c] rounded-lg border border-[#ba6a5a]/30">
                  <div className="text-xs text-[#e49385] font-semibold mb-2">✨ Pro Tips:</div>
                  <ul className="text-gray-400 text-xs space-y-1">
                    <li>• Click walls directly in 3D to cycle through colors</li>
                    <li>• Click the ceiling fan to switch between models</li>
                    <li>• Click the light bulb to toggle room lighting</li>
                    <li>• Use both 3D clicks and panel controls together</li>
                  </ul>
                </div>
              </div>
            </div>
